...
def test_get_request():
    response = client.get(f'/event_requests/{event_request_1["record_number"]}')
    assert response.status_code == 200
    assert response.json() == {"event_request": event_request_1}


def test_discard_request():
    response = client.delete(f'/event_requests/{event_request_1["record_number"]}')
    assert response.status_code == 200
