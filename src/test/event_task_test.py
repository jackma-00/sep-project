...
def test_add_task2():
    response = client.post("/tasks/", json=task_2)
    assert response.status_code == 200
    assert response.json() == {"project_reference": task_2["project_reference"]}


def test_add_task3():
    response = client.post("/tasks/", json=task_3)
    assert response.status_code == 200
...
def test_assignee_tasks():
    response = client.get(f'/tasks/projects/{task_3["assign_to"]}')
    assert response.status_code == 200
    assert response.json() == {"assignee_tasks": [task_1, task_3]}


def test_discard_assignee_tasks():
    response = client.delete(f'/tasks/projects/{task_3["assign_to"]}')
    assert response.status_code == 200
    assert response.json() == {"discard": "ok"}


def test_discard_project_tasks():
    response = client.delete(f'/tasks/{task_3["project_reference"]}')
    assert response.status_code == 200
    assert response.json() == {"discard": "ok"}
